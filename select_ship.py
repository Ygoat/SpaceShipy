...
class SelectShip():
    ...
    def __init__(self,screen:pygame.Surface):
        ...
        self.items_sur = self.__create_item(self.params)[0]
        self.items_rect = self.__create_item(self.params)[1]
        # 初期位置にセット
        self.__set_rect(screen)

    def show_texts(self,screen:pygame.Surface):
        selectship_pos = self.__text_place_position(self.text_sur.get_rect(),(screen.get_rect().centerx,screen.get_rect().top + 100))
        screen.blit(self.text_sur,selectship_pos)
    ...
    def __create_item(self,items:list[dict]) -> tuple[list[pygame.Surface],list[pygame.Rect]]:
        ...
        for i in range(0,len(items)):
            ...
            # pos_on_itemsur = (item_sur.get_rect().centerx-ship_rec.centerx,item_sur.get_rect().centery-ship_rec.centery)
            # ship_rec.topleft = pos_on_itemsur
            # ship_rec.clamp_ip(item_sur.get_rect())
